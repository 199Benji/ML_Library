...
const PublicBlog = () => {
  ...
  return (
    ...
    />
          ...
          /* Public Blog Grid View with Picture Cards */
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '1.5rem' }}>
              {posts.length === 0 ? (
                <div style={{ gridColumn: '1 / -1', textAlign: 'center', padding: '3rem', opacity: 0.6, background: 'var(--card-bg)', borderRadius: '12px' }}>
                  No public posts available yet. Check back soon!
                </div>
              ) : (
                posts.map(post => (
                  <motion.div 
                    ...
                    style={{ background: 'var(--card-bg)', borderRadius: '14px', border: '1px solid rgba(100, 255, 218, 0.15)', overflow: 'hidden', cursor: 'pointer', display: 'flex', flexDirection: 'column', boxShadow: '0 10px 25px rgba(0,0,0,0.3)' }}
                  >
                    {post.coverImageUrl && (
                    ...
                    <div style={{ padding: '1.25rem', display: 'flex', flexDirection: 'column', flex: 1, justifyContent: 'space-between' }}>
                      ...
                      {post.tags && post.tags.length > 0 && (
                        <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap', marginTop: '1.25rem' }}>
                          {post.tags.map(t => (
                            <span key={t} style={{ background: 'rgba(100, 255, 218, 0.1ode)', color: 'var(--accent-color)', padding: '2px 8px', borderRadius: '4px', fontSize: '0.7rem', fontWeight: '600' }}>#{t}</span>
                          ))}
                        </div>
                      )}
                    </div>
