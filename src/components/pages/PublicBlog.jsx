...
const BlogManager = ({ showToast }) => {
  ...
  const handleConfirmedDelete = async (postId, e) => {
    e.stopPropagation();
  ...
  return (
    <div style={{ marginTop: '2rem', paddingTop: '1rem', width: '100%', boxSizing: 'border-box' }}>
      <style>{`
      ...
      {viewingPost ? (
        ...
        <>
          ...
          {/* Compact Grid Layout matching Knowledge Base Segments */}
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '1.25rem', width: '100%', boxSizing: 'border-box' }}>
            {posts.length === 0 ? (
              <div style={{ gridColumn: '1 / -1', opacity: 0.6, fontSize: '0.9rem', padding: '1.5rem', background: 'var(--card-bg)', borderRadius: '12px', textAlign: 'center' }}>
              ...
              posts.map(post => (
                <motion.div 
                  ...
                  style={{ background: 'var(--card-bg)', borderRadius: '12px', border: '1px solid rgba(100, 255, 218, 0.15)', overflow: 'hidden', display: 'flex', flexDirection: 'column', boxShadow: '0 8px 20px rgba(0,0,0,0.2)', cursor: 'pointer', width: '100%', boxSizing: 'border-box' }}
                >
                  {post.coverImageUrl && (
                    ...
                    <div>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
                        ...
                        <div style={{ display: 'flex', gap: '8px' }}>
                          ...
                          <button onClick={(e) => handleConfirmedDelete(post.id, e)} title="Delete Post" style={{ background: 'transparent', border: 'none', color: '#ef4444', cursor: 'pointer', opacity: 0.8 }}>
                            <FiTrash2 size={15} />
                          </button>
                        </div>
                      </div>
          ...
          <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', background: 'rgba(5, 11, 20, 0.85)', backdropFilter: 'blur(5px)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1200, padding: '1rem', boxSizing: 'border-box' }}>
            <motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.98 }} style={{ background: 'var(--card-bg)', padding: '1.5rem 1.25rem', borderRadius: '16px', border: '1px solid rgba(239, 68, 68, 0.3)', width: '100%', maxWidth: '380px', textAlign: 'center', boxShadow: '0 20px 40px rgba(0,0,0,0.4)', boxSizing: 'border-box' }}>
              ...
              <div style={{ display: 'flex', gap: '0.75rem', width: '100%', boxSizing: 'border-box' }}>
                ...
                <button onClick={executeDelete} disabled={isDeleting} style={{ flex: 1, background: '#ef4444', color: '#ffffff', padding: '10px', borderRadius: '8px', fontWeight: '600', border: 'none', cursor: 'pointer', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '6px' }}>
